...
?>
  ...
  <div class="container p-3 pb-5">
    ...
    <section class="content-header sticky-top" style="height: 0px; z-index: -99;">
      <div class="row">
        <div class="card col-sm-18 p-3" style="margin-top: 60px">
          <div class="row">
            <div class="col-sm-18">
              ...
              <div class="item-info">
                <ul class="label-group">
                  ...
                  <a href="#">
                    <li class="label-item label-item-sm label-item-flush">
                      <span class="label-title">Delivery Fee:</span>
                      <span class="item-delivery-fee label-body">
                        RM <span>0.00</span>
                      </span>
                    </li>
                  </a>
    ...
    <section class="content-body">
      <div class="row">
        ...
        <main class="item-card card-group shadow col-sm-18">
          ...
          <div class="item-card-sidebar card col-sm-9">
            <div class="item-preview">
              <img class="card-img item-thumbnail" src="<?php echo $item['thumbnail']; ?>" alt="<?php echo ucwords($item['name']); ?>">
          ...
          <div class="item-card-main card col-sm-15">
            <ul class="list-group list-group-flush">
              ...
              <li class="item-info-group list-group-item">
                <h3 class="item-title">
                  <span class="badge badge-primary"><!--- TODO: Campaign Badge, e.g. Hot Sale, Featured ---></span>
                  <span class="title item-name"><?php echo ucwords($item['name']); ?></span>
                </h3>
              ...
              <li class="item-configuration list-group-item">
                <?php if ( ! empty($group['name'])) : ?>
                  <div class="list-group-heading p-0 text-xs">
                    <?php echo $group['name']; ?>
                  </div>
                <?php endif; ?>
                                  ...
                                  <div class="option-name has-addon has-input label-item">
                                    <div class="option-addon">
                                      ...
                                      <div class="addon-input addon-input-lg quantity">
                                        <input type="number" value="1" class="qty" step="1" min="1" max="99" maxLength="2" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);">
              ...
              <script>
                $(document).ready(function() {
                  ...
                  jQuery.updatePriceInfo = function updatePriceInfo() {
                    ...
                    $('.item-delivery-fee span').html(parseFloat(0).toFixed(2));

                    var postcode = $('#deliveryArea').find('.postcode').html();
                    ...
                    $.ajax({
                      ...
                      success: function(data){
                        if (data !== 'false') {
                          ...
                          $('.item-delivery-fee span').html($.number(info['delivery_fee'] * itemSet, 2, '.', ','));


                          switch (info['delivery_region']) {
                            case 'EM':
                              $('#deliveryEast').removeClass('d-none');
                              $('#deliveryWest').addClass('d-none');
                              break;
                            case 'WM':
                              $('#deliveryEast').addClass('d-none');
                              $('#deliveryWest').removeClass('d-none');
                              break;
                          }
                  ...
                  jQuery.itemAddCart = function itemAddCart() {
                    var itemSet = parseInt($('.attribute-item-set').find('input[type=number]').val());

                    var itemName = $('.item-name').html();
                    var itemThumbnail = $('.item-thumbnail').attr('src');

                    var itemTotalPrice = $('.item-total-price span').html();
                    var itemOriginalPrice = $('.item-original-price span').html();

                    var itemDiscountPercent = parseInt($('.item-discount-percent span').html());

                    itemTotalPrice = parseFloat(itemTotalPrice.replace(',', '')).toFixed(2);
                    itemOriginalPrice = parseFloat(itemOriginalPrice.replace(',', '')).toFixed(2);

                    itemTotalPrice = parseFloat(itemTotalPrice/itemSet).toFixed(2);
                    itemOriginalPrice = parseFloat(itemOriginalPrice/itemSet).toFixed(2);

                    $.ajax({
                      type: "POST",
                      url: "<?php echo site_url('cart/ajax/addCart/'); ?>",
                      data: {
                        itemID: <?php echo $item['item_id']; ?>,
                        itemSet: itemSet,
                        choices: choices,
                      ...
                      success: function(data){
                        if (data !== 'false') {
                          $('.cart-total-items').html(data['cartTotalItems']);
                          $('#modalAddCartSuccess').find('.item-name').html(data['name']);
                          $('#modalAddCartSuccess').find('.item-option-text').html(data['optionText']);
                          $('#modalAddCartSuccess').find('.item-qty span').html(data['qty']);
                          $('#modalAddCartSuccess').find('.item-subtotal span').html($.number(data['subtotal'], 2, '.', ','));
                          $('#modalAddCartSuccess').modal('show');
                        } else {
                          $('#modalAddCartFail').modal('show');
                        }
                      }
                    });
                  }
              ...
              <li class="item-action-group list-group-item">
                ...
                <div class="row py-2">
                  ...
                  <div class="col-sm-8">
                    <button id="btnAddCart" class="btn btn-lg btn-block bg-gradient bg-orange" disabled>Add to Cart</button>
                  </div>
                </div>
              </li>
            </ul>
        ...
        <aside class="item-widget col-sm-6 mt-3">
          ...
          <div class="item-tool shadow-sm sticky-top d-none">
            <ul class="list-group list-group-flush">
            ...
            <script>
              $(document).ready(function() {
                <?php if ( ! empty($item['postcode'])) : ?>
                  $.ajax({
                    type: "POST",
                    url: "<?php echo site_url('item/ajax/setDeliveryArea'); ?>",
                    data: {
                      postcode: <?php echo $item['postcode']; ?>,
                    },
                    success: function(data){
                      if (data !== 'false') {
                        ...
                        $('#deliveryArea').find('.state').html(data['state']);
                        $('#deliveryArea').find('.city').html(data['city']);
                        $('#deliveryArea').find('.postcode').html(data['postcode']);

                        $('#deliveryEdit').addClass('d-none');
                        $('#deliveryArea').removeClass('d-none');
                      }
                    }
                  });
                <?php endif; ?>

                $('#deliveryEditBtn').on('click', function() {
                  $('#deliveryEdit').removeClass('d-none');
                  $('#deliveryArea').addClass('d-none');
                });

                $('#deliverySaveBtn').on('click', function() {
                  var postcode = $('#deliveryEditPostcode').val();

                  if (postcode == '') {
                    $('#deliveryEditPostcode').addClass('is-invalid');
                  } else {
                    $.ajax({
                      type: "POST",
                      url: "<?php echo site_url('item/ajax/setDeliveryArea'); ?>",
                      ...
                      success: function(data){
                        if (data !== 'false') {
                          ...
                          $('#deliveryEditPostcode').val('');
                          $('#deliveryEditPostcode').removeClass('is-invalid');
          ...
          <div class="item-tool shadow-sm">
            <ul class="list-group list-group-flush">
              <li class="list-group-item">
                <div class="mb-3">
                  <small class="text-primary text-xs">
                    <b>Delivery Area</b>
                    <button id="deliveryEditBtn" class="btn btn-outline-light text-primary border-0 font-weight-bold" style="position: absolute; right: 5px; top: 5px">Edit</button>
                  </small>
                </div>
                <ul class="list-group list-group-flush">
                  <li class="list-group-item p-0" style="border: none">
                    <div id="deliveryEdit" class="row no-gutters">
                      <div class="col-sm-17" style="font-size: 1rem; font-weight: 500;">
                        <input type="text" class="form-control mb-2 mr-sm-2" id="deliveryEditPostcode" placeholder="Enter Postcode">
                        <div class="invalid-feedback">
                          Pleasae input a valid postcode.
                        </div>
                      </div>
                      <div class="col-sm-7 pl-2">
                        <button id="deliverySaveBtn" type="submit" class="btn btn-block btn-primary">Save</button>
                      </div>
                    </div>

                    <div id="deliveryArea" class="row d-none">
                      <div class="col-sm-4 text-center p-1">
                        <i class="fas fa-map-marker-alt"></i>
                      </div>
                      <div class="col-sm-20" style="font-size: 1rem; font-weight: 500;">
                        <span class="state"></span>, <span class="city"></span>, <span class="postcode"></span>
                      </div>
                    </div>
                  </li>
                </ul>
              ...
              <li class="list-group-item pb-0">
                ...
                <ul class="list-group list-group-flush">
                  <li id="deliveryWest" class="list-group-item py-1 px-0" style="border: none">
                    <div class="row">
                      <div class="col-sm-4 text-center p-1">
                  ...
                  <li id="deliveryEast" class="list-group-item py-1 px-0" style="border: none">
                    <div class="row">
                      <div class="col-sm-4 text-center p-1">
                      ...
                      <div class="col-sm-20" style="font-size: 1rem; font-weight: 500;">
                        Standard Delivery
                        <p class="text-primary"><small style="font-weight: 500;">within East Malaysia</small></p>
                      </div>
                    </div>
                  </li>
                </ul>
  ...
  <!--- Modal Add Cart --->
  <div class="modal fade" id="modalAddCartSuccess" tabindex="-1" role="dialog" aria-labelledby="modalAddCartSuccessTitle" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">

        <div class="modal-header">
          <h5 class="modal-title mx-auto" id="modalAddCartSuccessTitle"><span></span>Item has been added to your shopping cart</h5>
        </div>
        ...
        <div class="modal-body">
          <div class="row no-gutters">
            <div class="col-18">
              <figure class="media m-0">
                ...
                <figcaption class="media-body">
                  <h6 class="item-name pl-2"></h6>
                  <p class="item-info">
                    <ul class="item-option-text label-group">
                    </ul>
                  </p>
                </figcaption>
              </figure>
            ...
            <div class="col-6 text-right">
              <span class="item-total-price h6">RM <span data-value-loaded>99999.99</span></span><br>
              <span class="item-qty">Qty: <span>0</span></span>
            </div>
          </div>
        </div>

        <div class="modal-footer justify-content-center">
          <button type="button" class="btn btn-secondary text-white" data-dismiss="modal">Continue Shopping</button>
          <a href="<?php echo site_url('cart'); ?>" class="btn btn-primary">Go To Cart</a>
        </div>
